"""
Flask application that uses Google Gen AI API for essay scoring.
...
# Initialize GenAI service (will use GEMINI_API_KEY environment variable)
genai_service = None
try:
    genai_service = GenAIService()
    print("GenAI service initialized successfully")
except Exception as e:
    print(f"Error initializing GenAI service: {str(e)}")
    print("GenAI features will be disabled")
...
# Essay scoring with GenAI API
@app.route('/api/score-essay', methods=['POST'])
def score_essay():
    """Score an essay using GenAI API"""
    if not genai_service:
        return jsonify({
            'error': 'GenAI service not available. Please set GEMINI_API_KEY environment variable.'
        }), 503
    ...
    rubric = data.get('rubric', genai_service.get_default_rubric())
    essay_set = data.get('essay_set', 1)
    
    try:
        # Score the essay
        result = genai_service.score_essay(data['essay'], prompt, rubric, essay_set)
        
        # Generate detailed feedback if requested
        if data.get('detailed_feedback', False):
            feedback = genai_service.generate_detailed_feedback(data['essay'], result)
...
# Simple essay scoring endpoint (fallback if GenAI API is not available)
@app.route('/api/simple-score', methods=['POST'])
def simple_score():
    """Provide a simple scoring response without requiring GenAI API"""
    data = request.json
